...
void SDV_CRYPTO_MLKEM_CTRL_API_TC001(int bits)
{
    ...
    int ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE + 100, &val, sizeof(val));
    ASSERT_EQ(ret, CRYPT_MLKEM_CTRL_NOT_SUPPORT);

    ret = CRYPT_EAL_PkeyCtrl(NULL, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val));
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);

    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, NULL, sizeof(val));
    ASSERT_EQ(ret, CRYPT_NULL_INPUT);

    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val) - 1);
    ASSERT_EQ(ret, CRYPT_INVALID_ARG);

    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val));
...
void SDV_CRYPTO_MLKEM_ENCAPS_API_TC001(int bits)
{
    ...
    uint32_t val = (uint32_t)bits;
    int32_t ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ret = CRYPT_EAL_PkeyEncapsInit(ctx, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);

    uint32_t cipherLen = 0;
    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_CT_LEN, &cipherLen, sizeof(cipherLen));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_SETPUB_API_TC002(int bits, Hex *testEK)
{
    ...
    CRYPT_EAL_PkeyCtx *ctx = CRYPT_EAL_PkeyNewCtx(CRYPT_PKEY_ML_KEM);
    uint32_t val = (uint32_t)bits;
    int ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ret = CRYPT_EAL_PkeyEncapsInit(ctx, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);

    uint32_t encapsKeyLen = 0;
    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_EK_LEN, &encapsKeyLen, sizeof(encapsKeyLen));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_SETPRV_API_TC002(int bits, Hex *testDK)
{
    ...
    ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_DK_LEN, &decapsKeyLen, sizeof(decapsKeyLen));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_KEYCMP_FUNC_TC001(int bits, Hex *r0, Hex *r1, Hex *r2, int isProvider)
{
    ...
    ASSERT_NE(ctx, NULL);
    uint32_t val = (uint32_t)bits;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val)), CRYPT_SUCCESS);
    ...
    val = (uint32_t)bits;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx2, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val)), CRYPT_SUCCESS);
    ASSERT_EQ(CRYPT_EAL_PkeyCmp(ctx, ctx2), CRYPT_MLKEM_KEY_NOT_EQUAL);
    ...
    ASSERT_NE(ctx3, NULL);
    val = (uint32_t)bits;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx3, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val)), CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_KEYGEN_FUNC_TC001(int bits, Hex *z, Hex *d, Hex *testEK, Hex *testDK, int isProvider)
{
    ...
    uint32_t encapsKeyLen = 0;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_EK_LEN, &encapsKeyLen, sizeof(encapsKeyLen)), CRYPT_SUCCESS);

    uint32_t decapsKeyLen = 0;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_DK_LEN, &decapsKeyLen, sizeof(decapsKeyLen)), CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_ENCAPS_DECAPS_FUNC_TC001(int bits, Hex *m, Hex *testEK, Hex *testDK, Hex *testCT, Hex *testSK, int isProvider)
{
    ...
    uint32_t cipherLen = 0;
    ASSERT_EQ(CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_GET_MLKEM_CT_LEN, &cipherLen, sizeof(cipherLen)), CRYPT_SUCCESS);
...
void SDV_CRYPTO_MLKEM_ABNORMAL_DECAPS_FUNC_TC001(int bits, Hex *m, Hex *testEK, Hex *testDK, Hex *testCT, Hex *testSK, Hex *changeCT)
{
    ...
    CRYPT_EAL_PkeyCtx *ctx = CRYPT_EAL_PkeyNewCtx(CRYPT_PKEY_ML_KEM);
    uint32_t val = (uint32_t)bits;
    uint32_t ret = CRYPT_EAL_PkeyCtrl(ctx, CRYPT_CTRL_SET_MLKEM_TYPE, &val, sizeof(val));
    ASSERT_EQ(ret, CRYPT_SUCCESS);
    ret = CRYPT_EAL_PkeyEncapsInit(ctx, NULL);
    ASSERT_EQ(ret, CRYPT_SUCCESS);
