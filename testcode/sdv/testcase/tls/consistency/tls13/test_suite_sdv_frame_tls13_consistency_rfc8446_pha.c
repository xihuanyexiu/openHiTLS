...
void UT_TLS_TLS13_RFC8446_CONSISTENCY_POSTHANDSHAKE_FUNC_TC001(void)
{
    ...
    ASSERT_TRUE(REC_Write(server->ssl, REC_TYPE_HANDSHAKE, sendBuf, sendLen) == HITLS_SUCCESS);
...
void UT_TLS_TLS13_RFC8446_CONSISTENCY_POSTHANDSHAKE_FUNC_TC010(void)
{
    ...
    ASSERT_TRUE(REC_Write(client->ssl, REC_TYPE_HANDSHAKE, sendBuf, sendLen) == HITLS_SUCCESS);
