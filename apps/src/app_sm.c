...
static char *GetUserFilePath(const char *workPath)
{
    ...
    int32_t ret = snprintf(path, APP_MAX_PATH_LEN, "%s/%s", workPath, HITLS_APP_SM_USER_FILE_NAME);
    if (ret < 0) {
...
static int32_t GetAppExpectHmac(const char *appPath, uint8_t *hmac, uint32_t *hmacLen)
{
    ...
    int32_t ret = sprintf(hmacPath, "%s.hmac", appPath);
    if (ret < 0) {
    ...
    char *nextTmp = NULL;
    do {
        tmp = strtok((char *)data.data, seps);
        if (tmp == NULL) {
