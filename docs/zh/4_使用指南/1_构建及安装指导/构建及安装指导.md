...
1. 下载openHiTLS代码，含业务代码、构建脚本、测试代码
   仓库地址：https://gitee.com/openhitls/openhitls-dev.git
2. openHiTLS依赖于libboundscheck库，构建之前需将其下载至openHiTLS/platform/Secure\_C
...
![image](./b40fa813-ca67-41a0-b553-72dac6059b47..png)
...
|--feature_config|指定编译特性配置文件|python3 configure.py --feature_config config/json|
|--compile_config|指定编译参数配置文件|python3 configure.py --feature_config config/json|
...
mkdir -p ./build
cd ./build
python3 ../configure.py #修改配置，详见3.2.1节
