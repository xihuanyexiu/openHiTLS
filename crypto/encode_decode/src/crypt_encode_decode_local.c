...
static int32_t PbkdfDeriveKey(int32_t iter, int32_t prfId,
...
int32_t ParseRsaPrikeyAsn1Buff(uint8_t *buff, uint32_t buffLen,
...
static int32_t EccEalKeyNew(BSL_ASN1_Buffer *ecParamOid,
...
static int32_t ParseEccPubkeyAsn1Buff(BSL_ASN1_BitString *bitPubkey,
...
static int32_t ParseEccPrikeyAsn1(BSL_ASN1_Buffer *encode,
...
int32_t ParseEccPrikeyAsn1Buff(uint8_t *buff, uint32_t buffLen,
    BSL_ASN1_Buffer *pk8AlgoParam, CRYPT_EAL_PkeyCtx **ealPriKey)
{
    uint8_t *tmpBuff = buff;
    uint32_t tmpBuffLen = buffLen;
...
static int32_t ParseEd25519PrikeyAsn1Buff(uint8_t *buff,
...
static int32_t ParseEd25519PubkeyAsn1Buff(uint8_t *buff,
...
static int32_t ParsePk8PrikeyAsn1(BSL_ASN1_Buffer *encode,
    CRYPT_EAL_PkeyCtx **ealPriKey)
{
    ...
    if (cid == BSL_CID_ED25519) {
        return ParseEd25519PrikeyAsn1Buff(octPriKey->buff, octPriKey->len, ealPriKey);
    }
#endif
    return CRYPT_DECODE_UNSUPPORTED_PKCS8_TYPE;
}

static int32_t ParseSubPubkeyAsn1(BSL_ASN1_Buffer *encode,
...
int32_t ParsePk8PriKeyBuff(BSL_Buffer *buff,
    CRYPT_EAL_PkeyCtx **ealPriKey)
{
...
static int32_t ParseDeriveKeyPrfAlgId(BSL_ASN1_Buffer *asn, int32_t *prfId)
{
    ...
    if (asn->len != 0) {
        BSL_ASN1_Buffer algoId[2] = {0};  // 2 algoid buffer.
...
static int32_t ParseEncDataAsn1(BslCid symAlg, EncryptPara *encPara,
    const BSL_Buffer *pwd, BSL_Buffer *decode)
{
    ...
    ret = PbkdfDeriveKey(iter, prfId, &salt, pwd->data, pwd->dataLen, &keyBuff);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    if (encPara->enData->dataLen != 0) {
        ...
        ret = DecryptEncData(encPara->ivData, encPara->enData, symAlg, false, &keyBuff,
            output, &dataLen);
        if (ret != CRYPT_SUCCESS) {
            (void)memset_s(key, sizeof(key), 0, sizeof(key));
            BSL_SAL_Free(output);
...
int32_t ParsePk8EncPriKeyBuff(BSL_Buffer *buff,
    const BSL_Buffer *pwd, CRYPT_EAL_PkeyCtx **ealPriKey)
{
    ...
    ret = ParseEncDataAsn1(symId, &encPara, pwd, &decode);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
...
int32_t CRYPT_EAL_ParseAsn1SubPubkey(uint8_t *buff,
...
static int32_t EncodeDeriveKeyParam(CRYPT_Pbkdf2Param *param, BSL_Buffer *encode,
    BSL_Buffer *salt)
{
    ...
    BSL_Buffer *unEncrypted, BSL_Buffer *salt, BSL_ASN1_Buffer *asn1)
{
    ...
    do {
        ...
        ret = PbkdfDeriveKey(pkcsParam->itCnt, pkcsParam->hmacId, salt, pkcsParam->pwd,
        ...
        BSL_Buffer ivData = {asn1[CRYPT_PKCS_ENCPRIKEY_SYMIV_IDX].buff, asn1[CRYPT_PKCS_ENCPRIKEY_SYMIV_IDX].len};
        ret = DecryptEncData(&ivData, &enData, pkcsParam->symId, true, &keyBuff, output,
            &pkcsDataLen);
        if (ret != CRYPT_SUCCESS) {
            break;
        }
...
static inline void DeinitRsaPrvCtx(CRYPT_EAL_PkeyPrv *rsaPrv)
{
...
#ifdef HITLS_CRYPTO_KEY_EPKI
int32_t EncodePk8EncPriKeyBuff(CRYPT_EAL_PkeyCtx *ealPriKey,
...
static int32_t ParsePKCS7EncryptedContentInfo(BSL_Buffer *encode,
    const uint8_t *pwd, uint32_t pwdlen, BSL_Buffer *output)
{
    ...
    ret = ParseEncDataAsn1(symId, &encPara, &pwdBuffer, output);
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    return ret;
...
int32_t CRYPT_EAL_ParseAsn1PKCS7EncryptedData(BSL_Buffer *encode,
    const uint8_t *pwd, uint32_t pwdlen, BSL_Buffer *output)
{
...
/**
 * @brief Encoding PKCS7-EncryptData: Only the buffer of PBES2 + PBKDF2 can be encoded.
 *        For details, see CheckEncodeParam.
 *
 * @param data [IN] Plaintext to be encrypted and encoded
 * @param encodeParam [IN] Parameters
 * @param encode [OUT] Encoded data. The memory is allocated by this interface.
 *
 * @return CRYPT_SUCCESS: success
 * @return other: fail
 */
static int32_t EncodePKCS7EncryptedContentInfo(BSL_Buffer *data,
    const CRYPT_EncodeParam *encodeParam, BSL_Buffer *encode)
{
    /* EncAlgid */
    int32_t ret = CheckEncodeParam(encodeParam);
...
int32_t CRYPT_EAL_EncodePKCS7EncryptDataBuff(BSL_Buffer *data,
