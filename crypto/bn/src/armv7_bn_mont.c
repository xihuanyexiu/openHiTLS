...
{
    if (mont->mSize > 1) {
        // ARMV7 assembly instructions will be used only when the num of blocks is greater than 8 and be a multiple of 8
        if (mont->mSize % 8 == 0 && mont->mSize > 8) {
            MontMul_Armv7(r, r, r, mont->mod, mont->k0, mont->mSize);
        } else {
    ...
    BN_Optimizer *opt, bool consttime)
{
    if (mont->mSize > 1) {
        // ARMV7 assembly instructions will be used only when the num of blocks is greater than 8 and be a multiple of 8
        if (mont->mSize % 8 == 0 && mont->mSize > 8) {
            MontMul_Armv7(r, a, b, mont->mod, mont->k0, mont->mSize);
        } else {
...
int32_t MontEncBin(BN_UINT *r, BN_Mont *mont, BN_Optimizer *opt, bool consttime)
{
    if (mont->mSize > 1) {
        // ARMV7 assembly instructions will be used only when the num of blocks is greater than 8 and be a multiple of 8
        if (mont->mSize % 8 == 0 && mont->mSize > 8) {
            MontMul_Armv7(r, r, mont->montRR, mont->mod, mont->k0, mont->mSize);
        } else {
...
void Reduce(BN_UINT *r, BN_UINT *x, const BN_UINT *one, const BN_UINT *m, uint32_t mSize, BN_UINT m0)
{
    ...
    if (mSize % 8 == 0 && mSize > 8) { // here, miSzie > 8 and armv7 instruction is used when the mSize % 8 == 0.
        MontMul_Armv7(r, x, one, m, m0, mSize);
    } else {
