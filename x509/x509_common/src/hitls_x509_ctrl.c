...
{
    CRYPT_RsaPadType *inPad = (CRYPT_RsaPadType *)val;
    if (privKey == NULL || val == NULL || valLen != sizeof(int32_t) ||
        (*inPad != CRYPT_PKEY_EMSA_PKCSV15 && *inPad != CRYPT_PKEY_EMSA_PSS)) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_INVALID_PARAM);
        return HITLS_X509_ERR_INVALID_PARAM;
    }
    CRYPT_PKEY_AlgId pkeyId = CRYPT_EAL_PkeyGetId(privKey);
    if (pkeyId != CRYPT_PKEY_RSA) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_SET_RSA_PAD);
        return HITLS_X509_ERR_SET_RSA_PAD;
    }

    CRYPT_RsaPadType pad;
    int32_t ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_PADDING, &pad, sizeof(CRYPT_RsaPadType));
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    ...
    if (pad == 0) {
        return CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_SET_RSA_PADDING, inPad, sizeof(CRYPT_RsaPadType));
    }
    if (pad != *inPad) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_SET_RSA_PAD_DIFF);
        return HITLS_X509_ERR_SET_RSA_PAD_DIFF;
    }
...
{
    if (privKey == NULL || val == NULL || valLen != sizeof(CRYPT_RSA_PssPara)) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_INVALID_PARAM);
        return HITLS_X509_ERR_INVALID_PARAM;
    }

    CRYPT_RSA_PssPara *inPara = (CRYPT_RSA_PssPara *)val;
    CRYPT_RSA_PssPara para = {0};
    CRYPT_RsaPadType pad;
    // pkey id must be rsa
    CRYPT_PKEY_AlgId pkeyId = CRYPT_EAL_PkeyGetId(privKey);
    if (pkeyId != CRYPT_PKEY_RSA) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_SET_RSAPSS_PARA);
        return HITLS_X509_ERR_SET_RSAPSS_PARA;
    }

    // padding mode must be rsapss
    int32_t ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_PADDING, &pad, sizeof(CRYPT_RsaPadType));
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    ...
    // If mdId exists, it must be the same as input.
    ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_MD, &para.mdId, sizeof(BslCid));
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        return ret;
    }
    if (para.mdId != (CRYPT_MD_AlgId)BSL_CID_UNKNOWN && para.mdId != inPara->mdId) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_MD_NOT_MATCH);
        return HITLS_X509_ERR_MD_NOT_MATCH;
    }
    // If mgfId exists, it must be the same as input.
    ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_MGF, &para.mgfId, sizeof(BslCid));
    if (ret != CRYPT_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
    }
    if (para.mgfId != (CRYPT_MD_AlgId)BSL_CID_UNKNOWN && para.mgfId != inPara->mgfId) {
        BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_MGF_NOT_MATCH);
        return HITLS_X509_ERR_MGF_NOT_MATCH;
    }

    return CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_SET_RSA_EMSA_PSS, inPara, sizeof(CRYPT_RSA_PssPara));
}

static int32_t ProcRsaSignInfo(CRYPT_EAL_PkeyCtx *privKey, CRYPT_MD_AlgId mdId, HITLS_X509_Asn1AlgId *x509Alg)
{
    CRYPT_RsaPadType pad;
    int32_t ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_PADDING, &pad, sizeof(CRYPT_RsaPadType));
    if (ret != CRYPT_SUCCESS) {
    ...
    CRYPT_MD_AlgId pssMd;
    switch ((int32_t)pad) {
        case CRYPT_PKEY_EMSA_PSS:
            ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_MD, &pssMd, sizeof(BslCid));
            if (ret != CRYPT_SUCCESS) {
                BSL_ERR_PUSH_ERROR(ret);
                return ret;
            }
            ...
            x509Alg->rsaPssParam.mdId = pssMd;
            ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_SALT, &x509Alg->rsaPssParam.saltLen, sizeof(int32_t));
            if (ret != CRYPT_SUCCESS) {
                BSL_ERR_PUSH_ERROR(ret);
                return ret;
            }
            ret = CRYPT_EAL_PkeyCtrl(privKey, CRYPT_CTRL_GET_RSA_MGF, &x509Alg->rsaPssParam.mgfId, sizeof(BslCid));
            if (ret != CRYPT_SUCCESS) {
                BSL_ERR_PUSH_ERROR(ret);
                return ret;
            }
        ...
        case CRYPT_PKEY_EMSA_PKCSV15:
            ...
            CRYPT_RSA_PkcsV15Para pkcsPara = {mdId};
            return CRYPT_EAL_PkeyCtrl(
                privKey, CRYPT_CTRL_SET_RSA_EMSA_PKCSV15, &pkcsPara, sizeof(CRYPT_RSA_PkcsV15Para));
        default:
            BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_RSA_PAD);
            return HITLS_X509_ERR_RSA_PAD;
    }
}
...
int32_t HITLS_X509_SetSignAlgInfo(CRYPT_EAL_PkeyCtx *privKey, CRYPT_MD_AlgId mdId, HITLS_X509_Asn1AlgId *x509Alg)
{
    ...
    switch (pkeyId) {
        case CRYPT_PKEY_RSA:
            return ProcRsaSignInfo(privKey, mdId, x509Alg);
        case CRYPT_PKEY_ECDSA:
        case CRYPT_PKEY_SM2:
            x509Alg->algId = BSL_OBJ_GetSignIdFromHashAndAsymId((BslCid)pkeyId, (BslCid)mdId);
            if (x509Alg->algId == BSL_CID_UNKNOWN) {
                BSL_ERR_PUSH_ERROR(HITLS_X509_ERR_ENCODE_SIGNID);
                return HITLS_X509_ERR_ENCODE_SIGNID;
            }
