...
HITLS_X509_Csr *HITLS_X509_CsrNew(void)
{
    ...
    attributes = BSL_LIST_New(sizeof(HITLS_X509_AttrEntry));
    if (attributes == NULL) {
        BSL_ERR_PUSH_ERROR(BSL_MALLOC_FAIL);
        goto ERR;
    }
...
static int32_t ParseCertRequestInfo(BSL_ASN1_Buffer *asnArr, HITLS_X509_Csr *csr)
{
    ...
    /* attributes */
    ret = HITLS_X509_ParseAttrList(&asnArr[HITLS_X509_CSR_REQINFO_ATTRS_IDX], csr->reqInfo.attributes);
    if (ret != BSL_SUCCESS) {
        BSL_ERR_PUSH_ERROR(ret);
        goto ERR;
    }
